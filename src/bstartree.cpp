...
void BStarTree<TypeTrait>::remove(const DataType &value){

}
template <class TypeTrait>
typename Tree<TypeTrait>::iterator BStarTree<TypeTrait>::begin() {
  return iterator(this->root);
}

template <class TypeTrait>
typename Tree<TypeTrait>::iterator BStarTree<TypeTrait>::end() {
  return Tree<TypeTrait>::iterator(nullptr);
}

template <class TypeTrait> BStarTree<TypeTrait>::BStarTree() : Tree<TypeTrait>() {
  this->root = std::make_shared<NodeType>();
}

template <class TypeTrait>
void BStarTree<TypeTrait>::insert(const DataType &value) {
...
BStarTree<TypeTrait>::insert(NodePtr ptr, const DataType &value) {
...
template <class TypeTrait> void BStarTree<TypeTrait>::split(NodePtr ptr, int pos) {

  constexpr int half_order = (this->TREE_ORDER * 2) / 3;
...
template <class TypeTrait>
void BStarTree<TypeTrait>::split_root(NodePtr ptr, const DataType &value) {
...
BStarTree<TypeTrait>::find(const DataType &value) const {
  return find(this->root, value);
}

template <class TypeTrait>
typename Tree<TypeTrait>::iterator
BStarTree<TypeTrait>::find(NodePtr ptr, const DataType &value) const {
  if (ptr == nullptr)
...
template <class TypeTrait> void BStarTree<TypeTrait>::print() const {
  print(this->root, 0);
  std::cout << "________________________\n";
}

template <class TypeTrait>
void BStarTree<TypeTrait>::print(NodePtr ptr, int level) const  {
  // TODO: Optimize
  if (ptr == nullptr) {
    return;
  }

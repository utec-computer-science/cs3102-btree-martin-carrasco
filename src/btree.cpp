...
BTree<TypeTrait>::insert(NodePtr ptr, const DataType &value) {
  ...
  /*auto it =
      std::find_if(ptr->data.begin(), ptr->data.end(),
                   [&value](const auto& current_value) { return value >= current_value; });
...
template <class TypeTrait> void BTree<TypeTrait>::split(NodePtr ptr, int pos) {
  ...
  NodePtr child1 = std::make_shared<Node>();
...
void BTree<TypeTrait>::split_root(NodePtr ptr, const DataType &value) {
  ...
  NodePtr lhc = std::make_shared<Node>();
  NodePtr rhc = std::make_shared<Node>();
  ...
  std::copy_n(node_in_overflow->children.begin() + half_order + 1, half_order + 1,
              rhc->children.begin());
...
template <class TypeTrait> 
typename BTree<TypeTrait>::iterator BTree<TypeTrait>::find(const DataType& value) const { 
    return find(root, value);
}

template <class TypeTrait> 
typename BTree<TypeTrait>::iterator BTree<TypeTrait>::find(NodePtr ptr, const DataType& value) const { 
    if(ptr == nullptr)
        return iterator(nullptr);

...
template <class Trait> struct BPTreeNode : public Node<Trait> {
  int count = 0;
	bool is_leaf = false;
	std::shared_ptr<BPTreeNode> next;
	std::shared_ptr<BPTreeNode> prev;
  ...
  bool is_overflow() const { return count > BTREE_ORDER; }
	bool is_underflow() const { return count < BTREE_ORDER / 2;}
...
template <class T, int Order = 3> struct BPTreeTrait {
	static constexpr int TREE_ORDER = 3;
  using DataType = T;
  using DataContainer = typename std::array<T,TREE_ORDER  + 1>;
  using Node = BTreeNode<BTreeTrait<T>>;
